...
rule sciclone_all:
    input: expand("results/sciclone_facets/{patient}.sciclone.tsv", patient = patients)


rule run_sciclone_facets:
    input:
        vcf = "results/filtered_vcf/{patient}.vcf",
        cnv = lambda wildcards: ["results/facets/%s.csv" % sample
            for sample in patient_sciclone[wildcards.patient]["tumor_samples"]]
    output: "results/sciclone_facets/{patient}.sciclone.tsv"
    params:
