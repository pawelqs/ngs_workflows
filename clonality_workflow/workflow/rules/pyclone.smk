...
rule pyclone_all:
    input:
        expand("results/pyclone_facets/{patient}.table_{table}.tsv", patient=patients, table=["cluster", "loci"]),
        expand("results/pyclone_facets/{patient}/tried.plot", patient=patients)
        # expand("results/{patient}/table_{table}.tsv", patient=patients, table=pyclone_tables)
        # expand("results/{patient}/clusters_{plot_type}.png", patient=patients, plot_type=pyclone_cluster_plots),
        # expand("results/{patient}/loci_{plot_type}.png", patient=patients, plot_type=pyclone_loci_plots)


def pyclone_get_cnv_file(method, patient, sample):
    sample_id = "%s_%s" % (patient, sample)
    if method == "titan":
        return "results/titan_optimal/%s/%s.segs.tsv" % (sample_id, sample_id)
    elif method == "facets":
        return "results/facets/%s.csv" % sample_id

rule pyclone_parse_tsv:
    input:
        vcf = "results/filtered_vcf/{patient}.vcf",
        cnv = lambda wildcards: pyclone_get_cnv_file(wildcards.cnv_method, wildcards.patient, wildcards.sample)
    output: "results/pyclone_{cnv_method}/{patient}/input/{patient}_{sample}.tsv"
    params:
...
rule pyclone_setup_facets:
    input: 
        purity_files = lambda wildcards: ["results/facets/%s.csv" % sample 
            for sample in patient_pyclone[wildcards.patient]["tumor_samples"]],
        tsv_files = lambda wildcards: ["results/pyclone_facets/%s/input/%s.tsv" % (wildcards.patient, sample) 
            for sample in patient_pyclone[wildcards.patient]["tumor_samples"]]
    output: "results/pyclone_facets/{patient}/input/samples_info.txt"
    run:
...
rule pyclone_setup_titan:
    input: 
        purity_files = lambda wildcards: ["results/titan_optimal/%s/%s.params.txt" % (sample, sample) 
            for sample in patient_pyclone[wildcards.patient]["tumor_samples"]],
        tsv_files = lambda wildcards: ["results/pyclone_titan/%s/input/%s.tsv" % (wildcards.patient, sample) 
            for sample in patient_pyclone[wildcards.patient]["tumor_samples"]]
    output: "results/pyclone_titan/{patient}/input/samples_info.txt"
    run:
        samples = patient_pyclone[wildcards.patient]["tumor_samples"]
...
rule pyclone_setup_analysis:
    input: "results/pyclone_{cnv_method}/{patient}/input/samples_info.txt"
    output: "results/pyclone_{cnv_method}/{patient}/config.yaml"
...
rule pyclone_run_analysis:
    input: "results/pyclone_{cnv_method}/{patient}/config.yaml"
    output: "results/pyclone_{cnv_method}/{patient}/trace/labels.tsv.bz2"
    conda: "../envs/pyclone_env.yml"
...
rule pyclone_build_cluster_table:
    input: 
        config = "results/pyclone_{cnv_method}/{patient}/config.yaml",
        labels = "results/pyclone_{cnv_method}/{patient}/trace/labels.tsv.bz2"
    output: "results/pyclone_{cnv_method}/{patient}/table_cluster.tsv"
    conda: "../envs/pyclone_env.yml"
...
rule pyclone_build_loci_table:
    input: 
        config = "results/pyclone_{cnv_method}/{patient}/config.yaml",
        labels = "results/pyclone_{cnv_method}/{patient}/trace/labels.tsv.bz2"
    output: "results/pyclone_{cnv_method}/{patient}/table_loci.tsv"
    conda: "../envs/pyclone_env.yml"
...
rule pyclone_try_plot:
    input: 
        config = "results/pyclone_{cnv_method}/{patient}/config.yaml",
        labels = "results/pyclone_{cnv_method}/{patient}/trace/labels.tsv.bz2"
    output: "results/pyclone_{cnv_method}/{patient}/tried.plot"
    params: 
...
#         config = "results/pyclone_{cnv_method}/{patient}/config.yaml",
#         labels = "results/pyclone_{cnv_method}/{patient}/trace/labels.tsv.bz2"
#     output: "results/pyclone_{cnv_method}/{patient}/clusters_{plot_type}.png"
#     conda: "../envs/pyclone_env.yml"
...
#         config = "results/pyclone_{cnv_method}/{patient}/config.yaml",
#         labels = "results/pyclone_{cnv_method}/{patient}/trace/labels.tsv.bz2"
#     output: "results/pyclone_{cnv_method}/{patient}/loci_{plot_type}.png"
#     conda: "../envs/pyclone_env.yml"
...
rule pyclone_copy_results:
    input: 
        cluster_table = "results/pyclone_{cnv_method}/{patient}/table_loci.tsv",
        loci_table = "results/pyclone_{cnv_method}/{patient}/table_cluster.tsv"
    output: 
        cluster_table = "results/pyclone_{cnv_method}/{patient}.table_loci.tsv",
        loci_table = "results/pyclone_{cnv_method}/{patient}.table_cluster.tsv"
    shell: 
